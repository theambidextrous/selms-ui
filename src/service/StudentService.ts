...
export const fetchAllStudentsByParent = async (token : string | any, parent: string, page = 1, size = 1000): Promise<ApiResponse> => {
     try {
        const response: AxiosResponse<any> = await AxiosInstance.get(`/students/find-by-parent/${parent}?page=${page}&size=${size}`, {
            headers: { Authorization: `Bearer ${token}`}
        });
        return {
            success: true, 
...
export const fetchAllStudentsBySubject = async (token : string | any, subject:  string, page = 1, size = 1000): Promise<ApiResponse> => {
     try {
        const response: AxiosResponse<any> = await AxiosInstance.get(`/students/find-by-subject/${subject}?page=${page}&size=${size}`, {
            headers: { Authorization: `Bearer ${token}`}
        });
        return {
            success: true, 
