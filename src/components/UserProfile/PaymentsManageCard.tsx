...
import { addNewPayment, editPayment, fetchAllStudents } from "../../service";
import { onErrorToast, onSuccessToast } from "../../util";
import { PaymentsObject } from "../../pages/Payment";
import Input from "../form/input/InputField";


export default function PaymentsManageCard({ selection, onExport, onRefresh } : { selection: PaymentsObject, onExport: any, onRefresh: any}) {
  const bearerToken = useSelector(selectAccessToken) as string;
  const [students, setStudentsData] = useState<any[]>();
  const { isOpen, openModal, closeModal } = useModal();
  const editModal = useModal();

  const onCreatePayment = async (values : PaymentsObject) => {
    const resp = await addNewPayment(bearerToken, values);
    if(resp.success){
      closeModal();
      setTimeout(() => {
  ...
  const onEditPayment = async (values : any) => {
    const resp = await editPayment(selection.id as any, bearerToken, values);
    if(resp.success){
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <Formik
              ...
              validationSchema={CreatePaymentsSchema}
                    ...
                    <div className="mt-7">
                      ...
                      <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                        <div className="col-span-2 lg:col-span-1">
                            <Label>Amount</Label>
                            <Input onChange={handleChange('amount')} type="text" value={values.amount} />
                            {errors.amount && touched.amount ? (
                                <div className='text-error-400'>{errors.amount}</div>
                            ) : null}
                        </div>
                        <div className="col-span-2 lg:col-span-1">
                            <Label>Narration</Label>
                            <Input onChange={handleChange('remarks')} type="text" value={values.remarks} />
                            {errors.remarks && touched.remarks ? (
                                <div className='text-error-400'>{errors.remarks}</div>
                            ) : null}
                        </div>
                        <div className="col-span-2 lg:col-span-1">
                            <Label>Student</Label>
                            ...
                            { students?.map((f:any) => (<option value={f.id}>{ f.fname } { f.lname }</option>)) }
                            </select>
        ...
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              ...
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Update amount to keep the record up to date.
              </p>
            </div>
              <Formik
                    ...
                    <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
                      <div className="mt-7">
                        ...
                        <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                          <div className="col-span-2 lg:col-span-1">
                              <Label>Amount</Label>
                              <Input disabled onChange={handleChange('amount')} type="text" value={values.amount} />
                              {errors.amount && touched.amount ? (
                                  <div className='text-error-400'>{errors.amount}</div>
                              ) : null}
                          </div>
                          ...
                          <div className="col-span-2 lg:col-span-1">
                              <Label>Student</Label>
                              <select disabled value={values.student} onChange={handleChange('student')} className="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                              <option value={''}>Select option</option>
                              { students?.map((f:any) => (<option value={f.id}>{ f.fname } { f.lname }</option>)) }
                              </select>
                              {errors.student && touched.student ? (
...
export const CreatePaymentsSchema = Yup.object().shape({
  student: Yup.string().max(55, 'Too Long!').required('Required field'),
  remarks: Yup.string().max(255, 'Too Long!').required('Required field'),
  amount: Yup.string().max(10, 'Too Long!').required('Required field'),
});
