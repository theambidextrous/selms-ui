...
import { addNewStream, editStream, fetchAllForms, fetchAllTeachers } from "../../service";
import { onErrorToast, onSuccessToast } from "../../util";
import { StreamObject } from "../../pages/Academics/Streams";


export default function StreamManageCard({ selection, onExport, onRefresh } : { selection: StreamObject, onExport: any, onRefresh: any}) {
    ...
    const editModal = useModal();
      ...
      const onLoadPageData = async () => {
        ...
        const parents = await fetchAllTeachers(bearerToken);
        if(parents.success){
            setTeachersData(parents.data.data);
        }else{
          onErrorToast(parents.message);
        }
      }

    const onCreateStream = async (values : StreamObject) => {
        const resp = await addNewStream(bearerToken, values);
        if(resp.success){
        closeModal();
        setTimeout(() => {
            onSuccessToast('Stream created successfully!');
        }, 300);
        }else{
    ...
    const onEditStream = async (values : any) => {
        const resp = await editStream(selection.id as any, bearerToken, values);
        if(resp.success){
        editModal.closeModal();
        setTimeout(() => {
            onSuccessToast('Stream updated successfully!');
        }, 500);
        }else{
        onErrorToast(resp.message);
        }
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Add New Stream Information
            </h4>
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Add Stream details so that subjects/programs can be linked to it.
            </p>
          </div>
            <Formik
              initialValues={{
                        ...
                        <div className="mt-7">
                            ...
                            <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                                ...
                                <div className="col-span-2 lg:col-span-1">
                                   ...
                                   <select value={values.class_teacher} onChange={handleChange('class_teacher')} className="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                                        <option value={''}>Select option</option>
                                        { teachers?.map( p => (<option value={p.id}>{ p.fname } { p.lname }</option>))}
                                    </select>
                                    {errors.class_teacher && touched.class_teacher ? (
                                    <div className='text-error-400'>{errors.class_teacher}</div>
                                    ) : null}
                                </div>
                                <div className="col-span-2 lg:col-span-1">
                                    <Label>Stream Label</Label>
                                    <Input onChange={handleChange('label')} type="text" value={values.label} />
                                    {errors.label && touched.label ? (
                                        <div className='text-error-400'>{errors.label}</div>
                                    ) : null}
                                </div>
                            </div>
                        </div>
        ...
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              ...
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Update stream details to keep the record up to date.
              </p>
            </div>
              <Formik
...
export const CreateStreamSchema = Yup.object().shape({
    form: Yup.string().max(5, 'Too Long!').required('Required field'),
    name: Yup.string().max(50, 'Too Long!').required('Required field'),
    class_teacher: Yup.string().max(5, 'Too Long!').required('Required field'),
    label: Yup.string().max(100, 'Too Long!').required('Required field'),
});
