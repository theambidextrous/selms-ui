...
import { addNewAppUser, AppUserTypes, editAppUser } from "../../service/AppUsersService";


export default function AppUsersManageCard({ selection, onExport, onRefresh } : { selection: AppUserObject, onExport: any, onRefresh: any}) {
  const bearerToken = useSelector(selectAccessToken) as string;
  const { isOpen, openModal, closeModal } = useModal();
  const editModal = useModal();

  const onCreateAppUser = async (values : AppUserObject) => {
    const resp = await addNewAppUser(bearerToken, values);
    if(resp.success){
      closeModal();
      setTimeout(() => {
  ...
  const onEditAppUser = async (values : any) => {
    const resp = await editAppUser(selection.id as any, bearerToken, values);
    if(resp.success){
      editModal.closeModal();
      setTimeout(() => {
        onSuccessToast('User updated successfully!');
      }, 500);
    }else{
      onErrorToast(resp.message);
    }
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Add user details to admit them to the institution.
            </p>
          </div>
            <Formik
              initialValues={{
                    ...
                    <div className="mt-7">
                      ...
                      <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                        ...
                        <div className="col-span-2 lg:col-span-1">
                            ...
                            <select value={values.user_type} onChange={handleChange('user_type')} className="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            <option value={''}>Select option</option>
                            { AppUserTypes?.map(f => (<option value={f.value}>{ f.label } </option>)) }
                            </select>
                            {errors.user_type && touched.user_type ? (
                            <div className='text-error-400'>{errors.user_type}</div>
                            ) : null}
                        </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                          <div>
                            ...
                            <Input onChange={handleChange('password')} type="password" value={values.password} />
                            {errors.password && touched.password ? (
                              <div className='text-error-400'>{errors.password}</div>
                            ) : null}
                          </div>
                          
                          <div>
                            <Label>Re-enter password</Label>
                            <Input onChange={handleChange('c_password')} type="password" value={values.c_password} />
                            {errors.c_password && touched.c_password ? (
                              <div className='text-error-400'>{errors.c_password}</div>
                            ) : null}
                          </div>
                        </div>
        ...
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              ...
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Update user details to keep the record up to date.
              </p>
            </div>
              <Formik
                initialValues={{...selection, password: '', c_password: ''}}
                validationSchema={CreateTeacherSchema}
                onSubmit={onEditAppUser}
              >
                {({ errors, touched, handleSubmit, handleChange, values }) => (
                  <form className="flex flex-col">
                    <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
...
export const CreateTeacherSchema = Yup.object().shape({
  ...
  user_type: Yup.string().max(5, 'Too Long!').required('Required field'),
});
