...
import { addNewAssessmentGroup, editAssessmentGroup } from "../../service";
import { onErrorToast, onSuccessToast } from "../../util";
import { AssessmentGroupObject } from "../../pages/Performance/AssessmentGroups";

export default function AssessmentGroupManageCard({ selection, onExport, onRefresh } : { selection: AssessmentGroupObject, onExport: any, onRefresh: any}) {
  const bearerToken = useSelector(selectAccessToken) as string;
  const { isOpen, openModal, closeModal } = useModal();
  const editModal = useModal();

  const onCreateAssessmentGroup = async (values : AssessmentGroupObject) => {
    const resp = await addNewAssessmentGroup(bearerToken, values);
    if(resp.success){
      closeModal();
      setTimeout(() => {
        onSuccessToast('Group created successfully!');
      }, 300);
    }else{
  ...
  const onEditAssessmentGroup = async (values : any) => {
    const resp = await editAssessmentGroup(selection.id as any, bearerToken, values);
    if(resp.success){
      editModal.closeModal();
      setTimeout(() => {
        onSuccessToast('Group updated successfully!');
      }, 500);
    }else{
      onErrorToast(resp.message);
    }
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Add New Assessment Grouping Information
            </h4>
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Add Assessment Group so that you can group student performance.
            </p>
          </div>
            <Formik
              initialValues={{
              ...
              validationSchema={CreateAssessmentGroupingSchema}
              onSubmit={onCreateAssessmentGroup}
            >
        ...
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              ...
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Update Assessment Grouping details to keep the record up to date.
              </p>
            </div>
              <Formik
                initialValues={selection}
                validationSchema={CreateAssessmentGroupingSchema}
                onSubmit={onEditAssessmentGroup}
              >
                {({ errors, touched, handleSubmit, handleChange, values }) => (
                  <form className="flex flex-col">
                    <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
...
export const CreateAssessmentGroupingSchema = Yup.object().shape({
  name: Yup.string().min(3, 'Too Short!').max(255, 'Too Long!').required('Required field'),
});
