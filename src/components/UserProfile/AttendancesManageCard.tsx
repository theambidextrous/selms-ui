...
import { addNewPerformance, editPerformance, fetchAllStudents, fetchAllTimeTables } from "../../service";
import { onErrorToast, onSuccessToast } from "../../util";
import { PerformancesObject } from "../../pages/Performance/Performances";
import TextArea from "../form/input/TextArea";

export default function AttendancesManageCard({ selection, onExport, onRefresh } : { selection: PerformancesObject, onExport: any, onRefresh: any}) {
  ...
  const onCreateAttendance = async (values : PerformancesObject) => {
  ...
  const onEditAttendance = async (values : any) => {
  ...
  const onLoadPageData = async () => {
    ...
    const lessons = await fetchAllTimeTables(bearerToken);
    if(lessons.success){
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Add student scores for different exams
            </h4>
                        ...
                        <div className="mt-7">
                            ...
                            <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                                ...
                                <div className="col-span-2 lg:col-span-1">
                                    ...
                                    <option value={''}>Select option</option>
                                    { lessons?.map(f => (<option value={f.id}>{ f.name } </option>)) }
                                    </select>
      ...
      { selection && (
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
                Editing Performance Information
              </h4>
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
...
export const CreatePerformanceSchema = Yup.object().shape({
  student: Yup.string().max(6, 'Too Long!').required('Required field'),
  subject: Yup.string().max(6, 'Too Long!').required('Required field'),
  group: Yup.string().max(6, 'Too Long!').required('Required field'),
  mark: Yup.number().min(1, 'Score cannot be negative').max(100, 'Score cannot exceed 100!').required('Required field'),
  term: Yup.string().max(6, 'Too Long!').required('Required field'),
