...
export default function TermManageCard({ selection, onExport, onRefresh } : { selection: TermObject, onExport: any, onRefresh: any}) {
  ...
  const onCreateTerm = async (values : TermObject) => {
    ...
    if(resp.success){
      closeModal();
      setTimeout(() => {
        onSuccessToast('Term created successfully!');
      }, 300);
    }else{
  ...
  const onEditTerm = async (values : any) => {
    ...
    if(resp.success){
      ...
      setTimeout(() => {
        onSuccessToast('Term updated successfully!');
      }, 500);
    }else{
      onErrorToast(resp.message);
    }
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Add Term details so that subjects/programs can be linked to it.
            </p>
          </div>
            <Formik
              initialValues={{
                        ...
                        <div className="mt-7">
                            ...
                            <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                                ...
                                <div className="col-span-2 lg:col-span-1">
                                    ...
                                    <select value={values.year} onChange={handleChange('year')} className="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                                        <option value={''}>Select year</option>
                                        { TermYears.map( (y:any) => (<option value={y}>{y}</option>)) }
                                    </select>
                                    {errors.year && touched.year ? (
                                        <div className='text-error-400'>{errors.year}</div>
                                    ) : null}
                                </div>

                                <div className="col-span-2 lg:col-span-1">
                                    <Label>Starts On</Label>
                                    <Input onChange={handleChange('start')} type="date" value={values.start} />
                                    {errors.start && touched.start ? (
                                        <div className='text-error-400'>{errors.start}</div>
                                    ) : null}
                                </div>

                                <div className="col-span-2 lg:col-span-1">
                                    <Label>Ends On</Label>
                                    <Input onChange={handleChange('end')} type="date" value={values.end} />
                                    {errors.end && touched.end ? (
                                        <div className='text-error-400'>{errors.end}</div>
                                    ) : null}
                                </div>
                                ...
                                <div className="col-span-2 lg:col-span-1">
                                    <Switch
                                        ...
                                        onChange={(v) => {
                                            setFieldValue('is_current', v ? 1 : 0)
                                        }}
                                    />
                                    {errors.is_current && touched.is_current ? (
                                        <div className='text-error-400'>{errors.is_current}</div>
                                    ) : null}
                                </div>
      ...
      { selection && (
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
                Editing <b>{selection.label} {selection.year}</b> Information
              </h4>
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Update Term details to keep the record up to date.
              </p>
            </div>
              <Formik
