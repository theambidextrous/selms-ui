...
import { AddNewTeacher, EditTeacher } from "../../service";
...
export default function TeacherManageCard({ selection, onExport, onRefresh } : { selection: TeacherObject, onExport: any, onRefresh: any}) {
  ...
  const onCreateTeacher = async (values : TeacherObject) => {
    const resp = await AddNewTeacher(bearerToken, values);
    if(resp.success){
      closeModal();
      setTimeout(() => {
        onSuccessToast('Teacher created successfully!');
      }, 300);
    }else{
  ...
  const onEditTeacher = async (values : any) => {
    const resp = await EditTeacher(selection.id as any, bearerToken, values);
    if(resp.success){
      editModal.closeModal();
      setTimeout(() => {
        onSuccessToast('Teacher updated successfully!');
      }, 500);
    }else{
      onErrorToast(resp.message);
    }
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Add teacher details to admit them to the institution.
            </p>
          </div>
            <Formik
              initialValues={{
        ...
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              ...
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Update teacher details to keep the record up to date.
              </p>
            </div>
              <Formik
                initialValues={{...selection, password: '', c_password: ''}}
                validationSchema={CreateTeacherSchema}
                onSubmit={onEditTeacher}
              >
                {({ errors, touched, handleSubmit, handleChange, values }) => (
                  <form className="flex flex-col">
                    <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
...
export const CreateTeacherSchema = Yup.object().shape({
  ...
  phone: Yup.string().max(10, 'Too Long!').required('Required field'),
});
