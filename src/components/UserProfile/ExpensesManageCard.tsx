...
import { addNewExpense, editExpense } from "../../service";
...
export default function ExpensesManageCard({ selection, onExport, onRefresh } : { selection: ExpensesObject, onExport: any, onRefresh: any}) {
  const bearerToken = useSelector(selectAccessToken) as string;
  const [persons, setPersonData] = useState<any[]>();
  const { isOpen, openModal, closeModal } = useModal();
  const editModal = useModal();

  const onCreateExpense = async (values : ExpensesObject) => {
    const resp = await addNewExpense(bearerToken, values);
    if(resp.success){
      closeModal();
      setTimeout(() => {
  ...
  const onEditExpense = async (values : any) => {
    const resp = await editExpense(selection.id as any, bearerToken, values);
    if(resp.success){
  ...
  const onLoadPageData = async () => {
    const persons = await fetchAllAppUsers(bearerToken);
    if(persons.success){
        setPersonData(persons.data.data);
    }else{
        onErrorToast(persons.message);
    }
  }
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Manage expenses for the institution.
            </p>
          </div>
            <Formik
              initialValues={{
                    ...
                    <div className="mt-7">
                      ...
                      <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                        <div className="col-span-2 lg:col-span-1">
                            <Label>Amount</Label>
                            <Input onChange={handleChange('fee')} type="text" value={values.fee} />
                            {errors.fee && touched.fee ? (
                                <div className='text-error-400'>{errors.fee}</div>
                            ) : null}
                        </div>
                        <div className="col-span-2 lg:col-span-1">
                            <Label>Narration</Label>
                            <Input onChange={handleChange('narration')} type="text" value={values.narration} />
                            {errors.narration && touched.narration ? (
                                <div className='text-error-400'>{errors.narration}</div>
                            ) : null}
                        </div>
                        <div className="col-span-2 lg:col-span-1">
                            <Label>Person Linked</Label>
                            <select value={values.person} onChange={handleChange('person')} className="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            <option value={''}>Select option</option>
                            { persons?.map((f:any) => (<option value={f.id}>{ f.fname } { f.lname }</option>)) }
                            </select>
                            {errors.person && touched.person ? (
                            <div className='text-error-400'>{errors.person}</div>
                            ) : null}
                        </div>
                      </div>
                    </div>
        ...
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              ...
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Update expense to keep the record up to date.
              </p>
            </div>
              <Formik
...
export const CreateExpensesSchema = Yup.object().shape({
  person: Yup.string().max(55, 'Too Long!').required('Required field'),
  narration: Yup.string().max(255, 'Too Long!').required('Required field'),
  fee: Yup.string().max(10, 'Too Long!').required('Required field'),
});
