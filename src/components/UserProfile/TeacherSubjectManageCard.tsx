...
import { addNewTeacherSubject, editTeacherSubject, fetchAllSubjects, fetchAllTeachers } from "../../service";
import { onErrorToast, onSuccessToast } from "../../util";
import { TeacherSubjectObject } from "../../pages/TeacherSubjects";


export default function TeacherSubjectManageCard({ selection, onExport, onRefresh } : { selection: TeacherSubjectObject, onExport: any, onRefresh: any}) {
  ...
  const onCreateTeacherSubject = async (values : TeacherSubjectObject) => {
    ...
    if(resp.success){
      closeModal();
      setTimeout(() => {
        onSuccessToast('Assigned created successfully!');
      }, 300);
    }else{
  ...
  const onEditTeacherSubject = async (values : any) => {
    const resp = await editTeacherSubject(selection.id as any, bearerToken, values);
    if(resp.success){
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Assign teacher subjects based on their expeties and expereince.
            </p>
          </div>
            <Formik
              initialValues={{
                    ...
                    <div className="mt-7">
                      ...
                      <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                        <div className="col-span-2 lg:col-span-1">
                            <Label>Teacher</Label>
                            <select value={values.teacher} onChange={handleChange('teacher')} className="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3  dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            <option value={''}>Select option</option>
                            { teachers?.map((f:any) => (<option value={f.id}>{ f.fname } { f.lname }</option>)) }
                            </select>
                            {errors.teacher && touched.teacher ? (
                            <div className='text-error-400'>{errors.teacher}</div>
                            ) : null}
                        </div>
                        <div className="col-span-2 lg:col-span-1">
                            <Label>Subject</Label>
      ...
      { selection && (
        <Modal isOpen={editModal.isOpen} onClose={editModal.closeModal} className="max-w-[700px] m-4">
          <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            <div className="px-2 pr-14">
              <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
                Editing Teacher Subject
              </h4>
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Update teacher subject to keep the record up to date.
              </p>
            </div>
              <Formik
